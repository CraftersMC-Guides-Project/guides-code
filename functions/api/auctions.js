...
function parseNBTBytes(bytes) {
  // Basic NBT structure parsing
  // NBT format: TAG_Compound (0x0a) followed by string name, then tags
  const result = {};
  let offset = 0;
  ...
  try {
    ...
    if (bytes[0] === 0x0a) {
      offset = 1;
      // Read tag name length (2 bytes, big-endian)
 ...
 * @param {number} tagType - NBT tag type
 * @param {Uint8Array} bytes - All NBT bytes
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    return new Response(JSON.stringify(data), {
      headers: {
        'Content-Type': 'application/json',
        'Cache-Control': 'public, max-age=300', // 5 minute cache for auctions
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, OPTIONS'
      }
    });
